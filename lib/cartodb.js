...
function CartoDB(params) {
  ...
  this.api_url = this.api_url || resources.api_url.replace(':user', this.user);
...
CartoDB.prototype.query = function(_sql, args, fn) {
  ...
  debug('About to perform a query to %s', self.api_url);
  var req;
  if (sql.length > 2048 || isWriteQuery(sql)) {
    // use post for large queries
    req = request.post(this.api_url)
            .type("form")
            .send({ q: sql, api_key: this.api_key });
  } else {
    req = request.get(this.api_url)
            .type("form")
