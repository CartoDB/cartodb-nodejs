...
CartoDB.prototype.query = function(_sql, args) {
  ...
  if(sql.length > 2048 || this.isWriteQuery(sql))
    request
    .post(self.api_url)
    .set('host', 443)
    .set('Connection', 'keep-alive')
    .set('Content-type', 'application/x-www-form-urlencoded')
    .send({q: qs.escape(sql), api_key: self.api_key})
    .end(function(res){
      if(res.ok) self.emit('data', data);
      else self.emit('error', res.error);
    });
    ...
    .set('Connection', 'keep-alive')
    .set('Content-type', 'application/x-www-form-urlencoded')
    .query({q: qs.escape(sql), api_key: self.api_key})
    .end(function(res){
      if(res.ok) self.emit('data', data);
      else self.emit('error', res.error);
    });
...
function isWriteQuery = CartoDB.prototype.isWriteQuery = function(sql) {
