...
Import.prototype.stream = function(stream, options) {

  var self = this;

  var promise = new _Promise();
  if(!stream) {
    throw new TypeError('stream should not be null');
  }
  ...
  var reqOptions = {
    ...
    formData: {
      file:{
        //https://github.com/request/request/issues/1495
        value: stream,
        options:{
          // MAIN ROADBLOCK: https://github.com/CartoDB/cartodb-nodejs/issues/57
          // knownLength: 7508273,
  ...
  options.reqOptions = reqOptions;

  this._post('stream',options);
      ...
      function processGetResponsefunction(err, res, body) {
        ...
        try {
          ...
          if( body.state == 'complete' ) {
            ...
            options.promise.emit('_error', new Error(body.get_error_text || body.error_code));
